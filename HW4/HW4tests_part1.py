...
class HW4Sampletests_part1(unittest.TestCase):
    ...
    def test_getinterval1(self):
        #(HelloWorld) 4 3 getinterval
        opPush('(HelloWorld)')
        opPush(4)
        opPush(3)
        getinterval()
        self.assertEqual(opPop(),'(oWo)')
        self.assertTrue(len(opstack)==0)
        #getinterval will not push back the string onto the opstack
    ...
    def test_putinterval4(self):
        #(HelloWorld) dup 1 (alloween) putinterval
        opPush('(HelloWorld)')
        dup()
        opPush(1)
        opPush('(alloween!)')
        putinterval()
        self.assertEqual(opPop(), '(Halloween!)')
        self.assertTrue(len(opstack) == 0)
    ...
    def test_search3(self):
        ...
        opPush('(e)')
        search()
        self.assertTrue(opPop()==True and opPop()=='(Hallow)' and opPop()=='(e)' and opPop()=='(en)')
    ...
    def test_evaluateArray1(self):
        define('/x','(aba)')
        define('/y','(cdc)')
        # [1 x 1 y 1]
        # [x 1 1 add y 1 get]
        # [(aBCd) dup 1 x putinterval (aaba) eq]
        self.assertEqual(evaluateArray([1,'x',1,'y',1]), [1,'(aba)',1,'(cdc)',1])
        self.assertEqual(evaluateArray(['x',1,1,'add','y',1,'get']), ['(aba)',2,100])
        self.assertEqual(evaluateArray(['(aBCd)','dup',1,'x','putinterval','(aaba)','eq']), [True])
    ...
    def test_astore1(self):
        ...
        opPush([0, 0, 0, 0])  # the array doesn't necessarily neeed to have 0 values.
        astore()
        self.assertTrue(opPop()==[None,1,2,3])
            #-----------------------------------------------------
    ...
    def test_beginEnd1(self):
        #/x (abc) def 1 dict begin /x (bcd) def /x (cde) def end x
        opPush("/x")
        opPush('(abc)')
        psDef()
        opPush(1)
